##########################################
# RouterOS 7.20 hAP ax² - Dual ISP + PBR
# Default ISP2, Mobile Legends via ISP1
# Include DHCP Client + NTP Client (unicast, 3 servers)
##########################################
...
add address=172.16.0.2/24 interface=ether2 comment="ISP1 Telkom (Static)"
add address=172.8.45.2/24 interface=ether3 comment="ISP2 Fastlink (Static)"

# === DHCP CLIENT (Optional: enable if needed) ===
/ip/dhcp-client
# Uncomment baris di bawah ini jika ingin otomatis IP dari ISP
# add interface=ether2 use-peer-dns=no add-default-route=no comment="DHCP ISP1 Telkom"
# add interface=ether3 use-peer-dns=no add-default-route=no comment="DHCP ISP2 Fastlink"
...
/ip route
add dst-address=0.0.0.0/0 gateway=172.16.0.1 routing-table=to-ISP1 comment="Default via ISP1 Telkom"
add dst-address=0.0.0.0/0 gateway=172.8.45.1 routing-table=to-ISP2 comment="Default via ISP2 Fastlink"
add dst-address=0.0.0.0/0 gateway=172.8.45.1 comment="Main default ISP2"

# === NAT MASQUERADE ===
/ip firewall nat
add chain=srcnat out-interface=ether2 action=masquerade comment="NAT ISP1 Telkom"
add chain=srcnat out-interface=ether3 action=masquerade comment="NAT ISP2 Fastlink"

# === ADDRESS-LIST LOCAL (bypass local traffic) ===
/ip firewall address-list
add address=10.10.0.0/12 list=LOCAL comment="LAN/Hotspot subnet"
add address=172.0.0.0/8 list=LOCAL comment="Private/Local range"
...
# === MANGLE RULES FOR MOBILE LEGENDS ===
# TCP Split (max 15 port per rule)
...
# UDP Split (max 15 port per rule)
